...
const BlueJump = () => {
    ...
    useEffect(() => {
        ...
        // Start the animation
        Animated.loop(jumpAnimation).start();

        // Clean up the animation on unmount
        return () => jumpValue.removeAllListeners();
    }, []);
                    ...
                    transform: [
                        {
                            translateY: jumpValue.interpolate({
                                inputRange: [0, 0.5, 1],
                                outputRange: [0, -20, 0], // Adjust this value for desired jump height
                            }),
                        },
                    ],
                    height:25,
